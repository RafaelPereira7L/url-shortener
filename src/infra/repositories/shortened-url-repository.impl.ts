...
export class ShortenedUrlRepositoryImpl implements ShortenedUrlRepository {
  constructor(@InjectRepository(ShortenedUrl) private readonly shortenedUrlRepository: Repository<ShortenedUrl>) { }
  ...
  async update(shortenedUrl: ShortenedUrl): Promise<boolean> {
    const updateResult = await this.shortenedUrlRepository.update(shortenedUrl.id, shortenedUrl);
    return updateResult.affected === 1;
  }

  async delete(shortenedUrlId: string): Promise<boolean> {
    const deleteResult = await this.shortenedUrlRepository.softDelete({ id: shortenedUrlId });
  ...
  async findAllByUserId(userId: string): Promise<ShortenedUrl[]> {
    ...
    .loadRelationCountAndMap('shortenedUrl.clicksCount', 'shortenedUrl.clicks') // Carrega a contagem de cliques
    .select([
      'shortenedUrl.id',
